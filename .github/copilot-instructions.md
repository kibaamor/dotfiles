...
- `home/.chezmoidata/` holds structured data consumed by templates: package lists, external tool versions, and Windows optional features. Keep version keys aligned with names used in `.chezmoiexternal.yaml.tmpl`; `update-version.sh` generates keys by stripping letters, hyphens, and spaces from GitHub repo names.
